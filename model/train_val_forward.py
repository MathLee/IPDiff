...
def modification_train_val_forward(model: nn.Module, gt=None, image=None, seg=None, mode=None, **kwargs):
    ...
    if model.training:
        ...
        return model(gt, image, seg=seg, mode=mode, **kwargs)
    else:
        time_ensemble = kwargs.pop('time_ensemble') if 'time_ensemble' in kwargs else False
        gt_sizes = kwargs.pop('gt_sizes') if time_ensemble else None
        pred = model.sample(image, mode, **kwargs).detach().cpu()
        if time_ensemble:
