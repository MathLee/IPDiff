...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def forward(self, img, cond_img, seg=None, mode=None, extra_cond=None, *args, **kwargs): # img is gt, cond_img is RGB
        ...
        return self.p_losses(img, times, cond_img, seg, mode, extra_cond, *args, **kwargs)

    def p_losses(self, x_start, times, cond_img, seg=None, mode=None, extra_cond=None, noise=None, *args, **kwargs):
        noise = default(noise, lambda: torch.randn_like(x_start)) # 首先随机生成噪声
        ...
        # 噪声采样,注意这个是一次性完成的   
        # denoising_diffusion_pytorch.simple_diffusion line 658 is q_sample

        # predict and take gradient step
        model_out, pvt_out = self.model(torch.cat([x, extra_cond], dim=1), log_snr, cond_img,mode)
        # net.py line 612, log_snr is timestep
        ...
        if self.loss_type == 'l2':
            ...
            loss_diff = self.loss_type(model_out, target)
            loss_pvt = self.loss_type(pvt_out, target)
            return loss_diff + 0.5*loss_pvt

    @torch.no_grad()
    def sample(self, cond_img, mode=False, extra_cond=None, verbose=True):
    ...
    @torch.no_grad()
    def p_sample_loop(self, shape, cond_img, mode, extra_cond, verbose=True):
        self.history = []
        ...
        for i in tqdm(range(self.num_sample_steps), desc='sampling loop time step', total=self.num_sample_steps,
            ...
            img = self.p_sample(img, conditioning_features, mode, extra_cond, times, times_next)
    ...
    @torch.no_grad()
    def p_sample(self, x, cond, mode, extra_cond, time, time_next):
        batch, *_, device = *x.shape, x.device

        model_mean, model_variance = self.p_mean_variance(x=x, cond=cond, mode=mode, extra_cond=extra_cond, time=time,
                                                          time_next=time_next)
    ...
    def p_mean_variance(self, x, cond, mode, extra_cond, time, time_next):
        ...
        pred, _ = self.model.sample_unet(torch.cat([x, extra_cond], dim=1),
                                      batch_log_snr, cond, mode)
