...
class PyramidVisionTransformerImpr(nn.Module):
    ...
    def forward_features(self, x, timesteps, cond_img):
        ...
        # stage 1
        x, H, W = self.patch_embed1(cond_img, x) #Eq.6
        x = torch.cat([time_token, x], dim=1) #加入时间token
...
class CASA(nn.Module):
    ...
    def forward(self, x):
        x_real = x.real
        x_ca = x.mul(self.ca(x_real))
        x_ca_real = x_ca.real
        x_sa = x_ca.mul(self.sa(x_ca_real))
        x = x + x_sa
        return x

# Information Reconstruction-driven Attention Module (IRAM)
class Fre_enhance(nn.Module):
    def __init__(self, in_channels=256):
        super(Fre_enhance, self).__init__()

        # self.freq_nums = 0.5
        self.freq_nums = torch.sigmoid(nn.Parameter(torch.zeros(1, dtype=torch.float32), requires_grad=True))
        # nor_weights = F.softmax(self.freq_nums, dim=0)

        self.casa_low = CASA(input_dim=in_channels)
        self.casa_high = CASA(input_dim=in_channels)
    ...
    def forward(self, x):
        ...
        w, h = x.shape[-2:]
        line = int((w * h * self.freq_nums) ** .5 // 2)
        mask[:, :, w // 2 - line:w // 2 + line, h // 2 - line:h // 2 + line] = 1
        # frequency domain
        fft = torch.fft.fftshift(torch.fft.fft2(x.float()))
        # low frequency
        fft_low = fft * (1 - mask)
        fft_low_casa = self.casa_low(fft_low)
        fft_low_casa = torch.fft.ifftshift(fft_low_casa)
        inv_low = torch.fft.ifft2(fft_low_casa).real
        inv_low = self.relu(self.norm(torch.abs(inv_low)))
        # high frequency
        fft_high = fft * mask
        fft_high_casa = self.casa_high(fft_high)
        fft_high_casa = torch.fft.ifftshift(fft_high_casa)
        inv_high = torch.fft.ifft2(fft_high_casa).real
        inv_high = self.relu(self.norm(torch.abs(inv_high)))
        # fusing low frequency and high frequency with learning weight
        weight = 2 * F.softmax(self.weight, dim=0)
        fusion = inv_low * weight[0] + inv_high * weight[1]

        out = x + x.mul(self.SA(fusion))
...
class Decoder(Module):
    def __init__(self, dims, dim, class_num=2, mask_chans=1):
        ...
        embedding_dim = dim
        # 这里的conv是自己写的函数（见line442），是两个conv+relu层
        ...
        self.FreE4 = Fre_enhance(in_channels=embedding_dim)
        self.FreE3 = Fre_enhance(in_channels=embedding_dim)
        self.FreE2 = Fre_enhance(in_channels=embedding_dim)
        self.FreE1 = Fre_enhance(in_channels=embedding_dim)
        ...
        # 88->88
        self.down_pvt = ConvModule(in_channels=1, out_channels=embedding_dim, kernel_size=3, padding=1,
                                 norm_cfg=dict(type='BN', requires_grad=True))

        # 88->88
        self.down_1 = ConvModule(in_channels=embedding_dim * 3, out_channels=embedding_dim, kernel_size=3, padding=1,
                                 norm_cfg=dict(type='BN', requires_grad=True))
        #88->44
        self.down_2 = ConvModule(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=3, padding=1, stride=2,
                   norm_cfg=dict(type='BN', requires_grad=True))
        # 44->22
        self.down_3 = ConvModule(in_channels=embedding_dim * 2, out_channels=embedding_dim, kernel_size=3, padding=1, stride=2,
                                 norm_cfg=dict(type='BN', requires_grad=True))
        # 22->11
        self.down_4 = ConvModule(in_channels=embedding_dim * 2, out_channels=embedding_dim, kernel_size=3, padding=1, stride=2,
                                 norm_cfg=dict(type='BN', requires_grad=True))

        # 11->22
        self.up_4 = nn.Sequential(
            ConvModule(in_channels=embedding_dim * 2, out_channels=embedding_dim, kernel_size=3, padding=1,
                       norm_cfg=dict(type='BN', requires_grad=True)),
            ConvModule(in_channels=embedding_dim, out_channels=embedding_dim, kernel_size=3, padding=1,
                       norm_cfg=dict(type='BN', requires_grad=True)),
        ...
        self.pred_pvt = nn.Sequential(
            ConvModule(in_channels=embedding_dim * 4, out_channels=embedding_dim, kernel_size=1,
                       norm_cfg=dict(type='BN', requires_grad=True)),
            nn.Dropout(0.1),
            Conv2d(embedding_dim, self.num_classes, kernel_size=3, padding=1)
        )

# Information Perturbation Module (IPM)
    def mask_pixel(self, x, ratio):
        mask = torch.rand(x.shape[0], 1, x.shape[2], x.shape[3]).cuda()
        mask = torch.where(mask < ratio, 0., 1.).cuda()
        x = torch.mul(x, mask)
        return x

    def forward(self, inputs, timesteps, x, mode):
        t = self.time_embed(timestep_embedding(timesteps, self.time_embed_dim))

        c1, c2, c3, c4 = inputs

        ############## MLP decoder on C1-C4, Frequency enhancement###########
        n, _, h, w = c4.shape

        _c4 = self.linear_c4(c4).permute(0, 2, 1).reshape(n, -1, c4.shape[2], c4.shape[3])
        # c4_copy = resize(_c4, size=c1.size()[2:], mode='bilinear', align_corners=False)
        _c4 = self.FreE4(_c4)
        c4_copy = resize(_c4, size=c1.size()[2:], mode='bilinear', align_corners=False)

        _c3 = self.linear_c3(c3).permute(0, 2, 1).reshape(n, -1, c3.shape[2], c3.shape[3])
        # c3_copy = resize(_c3, size=c1.size()[2:], mode='bilinear', align_corners=False)
        _c3 = self.FreE3(_c3)
        c3_copy = resize(_c3, size=c1.size()[2:], mode='bilinear', align_corners=False)

        _c2 = self.linear_c2(c2).permute(0, 2, 1).reshape(n, -1, c2.shape[2], c2.shape[3])
        # c2_copy = resize(_c2, size=c1.size()[2:], mode='bilinear', align_corners=False)
        _c2 = self.FreE2(_c2)
        c2_copy = resize(_c2, size=c1.size()[2:], mode='bilinear', align_corners=False)
        ...
        # PVT saliency map generation
        x_cat = torch.cat([c1_copy, c2_copy, c3_copy, c4_copy], dim=1)
        x_pvt_pred = self.pred_pvt(x_cat)
        x_pvt = F.interpolate(x_pvt_pred, scale_factor=4, mode='bilinear', align_corners=False)
        ##############################################
        # Multi-Prior Guidance Denoising Network
        _x = [x]
        for blk in self.down:
            if isinstance(blk, ResnetBlock):
                ...
                x = blk(x)
        # x->x1 88->88
        x1_pvt = self.down_pvt(x_pvt_pred)
        x1 = self.down_1(torch.cat([_c1, x, x1_pvt], dim=1))
        if mode == True:
            x1 = self.mask_pixel(x1, 0.2)
        # x1->x2 88->44
        x2 = self.down_2(x1)
        if mode == True:
            x2 = self.mask_pixel(x2, 0.2)
        # x2->x3 44->22
        x3 = self.down_3(torch.cat([_c2, x2], dim=1))
        if mode == True:
            x3 = self.mask_pixel(x3, 0.2)
        # x3->x4 22->11, xc_4 11
        x4 = self.down_4(torch.cat([_c3, x3], dim=1))
        if mode == True:
            x4 = self.mask_pixel(x4, 0.2)
        xc_4 = torch.cat([_c4, x4], dim=1)
        ...
        x_4 = self.up_4(xc_4)
        # x_4->x_3 22->44
        x_3 = self.up_3(torch.cat([x3, x_4], dim=1))
        # x_3->x_2 44->88
        x_2 = self.up_2(torch.cat([x2, x_3], dim=1))

        x_1 = torch.cat([x1, x_2], dim=1)
        for blk in self.up:
            if isinstance(blk, ResnetBlock):
                x_1 = blk(x_1, t)
            else:
        ...
        x_noise = self.pred(x_1)
        # SimpleDiffSef.py的line 272计算loss
        return x_noise, x_pvt, c1, c2, c3, c4
...
class net(nn.Module):
    ...
    def forward(self, x, timesteps, cond_img, mode):
        features = self.backbone(x, timesteps, cond_img)
        #print(x[1,0,1]) 
        features, features_pvt, layer1, layer2, layer3, layer4 = self.decode_head(features, timesteps, x, mode)
        return features, features_pvt
    ...
    def sample_unet(self, x, timesteps, cond_img, mode):
        return self.forward(x, timesteps, cond_img, mode)
