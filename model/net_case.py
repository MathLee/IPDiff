...
class Decoder(Module):
    def __init__(self, dims, dim, class_num=2, mask_chans=1):
        ...
        self.casa_c4 = CASA(input_dim=embedding_dim)
        self.casa_c3 = CASA(input_dim=embedding_dim)
        self.casa_c2 = CASA(input_dim=embedding_dim)
        self.casa_c1 = CASA(input_dim=embedding_dim)
    ...
    def forward(self, inputs, timesteps, x):
        ...
        _c4 = self.casa_c4(_c4)
        _c4 = resize(_c4, size=c1.size()[2:], mode='bilinear', align_corners=False)

        _c3 = self.linear_c3(c3).permute(0, 2, 1).reshape(n, -1, c3.shape[2], c3.shape[3])
        _c3 = self.casa_c3(_c3)
        _c3 = resize(_c3, size=c1.size()[2:], mode='bilinear', align_corners=False)

        _c2 = self.linear_c2(c2).permute(0, 2, 1).reshape(n, -1, c2.shape[2], c2.shape[3])
        _c2 = self.casa_c2(_c2)
        _c2 = resize(_c2, size=c1.size()[2:], mode='bilinear', align_corners=False)

        _c1 = self.linear_c1(c1).permute(0, 2, 1).reshape(n, -1, c1.shape[2], c1.shape[3])
        _c1 = self.casa_c1(_c1)
